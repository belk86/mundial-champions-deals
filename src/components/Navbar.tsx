        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          <a href="#home" className="flex items-center">
            <span
              ...
              style={{
                color: '#FF9900',
                textShadow: '0 0 8px rgba(255,153,0,0.6), 0 0 16px rgba(255,153,0,0.4), 0 0 32px rgba(255,153,0,0.3), 0 0 48px rgba(255,153,0,0.2)',
              }}
            >
              FanEdge
            </span>
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <a key={item.key} href={item.href} onClick={(e) => handleNavClick(e, item.filter)}
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <Button variant="ghost" size="sm" onClick={() => setIsLangOpen(!isLangOpen)}
              ...
              <AnimatePresence>
                {isLangOpen && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                    className="absolute top-full mt-2 right-0 bg-card border border-border rounded-lg shadow-xl overflow-hidden min-w-[140px]">
                    {languages.map((lang) => (
                      <button key={lang.code} onClick={() => { setLanguage(lang.code); setIsLangOpen(false); }}
                        ...
                        <span className="font-english">{lang.label}</span>
            ...
            <Button variant="ghost" size="icon" className="md:hidden text-foreground hover:bg-secondary" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              ...
              <div className="py-4 space-y-2">
                {navItems.map((item) => (
                  <a key={item.key} href={item.href} onClick={(e) => { handleNavClick(e, item.filter); setIsMenuOpen(false); }}
