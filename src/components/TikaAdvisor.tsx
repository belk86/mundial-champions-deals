...
const TikaAdvisor = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: errorMessages[language] || errorMessages.en }]);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 20, scale: 0.9 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-black/70 text-xs">
                    {language === 'es' ? 'Asesora de compras IA' :
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${
                    msg.role === 'user' ? 'bg-amazon text-black rounded-br-md' : 'bg-muted text-foreground rounded-bl-md'
                  }`}>
            ...
            <div className="p-4 border-t border-border bg-card">
              <div className="flex gap-2">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
                  ...
                  disabled={isLoading} />
                <Button onClick={sendMessage} disabled={isLoading || !input.trim()} size="icon" className="rounded-full bg-amazon hover:bg-amazon-dark text-black">
