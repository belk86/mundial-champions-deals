export const translations = {
  en: {
    translation: {
      // Navigation
      nav: {
        home: "Home",
        fansGear: "Fans Gear",
        travelGuide: "Travel Guide",
        schedule: "World Cup Schedule",
        joinNow: "Join Now",
      },
      // Hero
      hero: {
        yourPremium: "Your Premium",
        worldCup2026: "World Cup 2026",
        destination: "Destination",
        subtitle: "Shop viral TikTok products, top-seller gear, and book your match day stays across USA, Canada & Mexico. Everything a true fan needs for the greatest show on earth.",
        shopGear: "Shop Fans Gear",
        planTrip: "Plan Your Trip",
        scrollExplore: "Scroll to explore",
      },
      // Products
      products: {
        sectionLabel: "Featured Products",
        title: "Fans Gear Collection",
        subtitle: "Curated selection of viral TikTok products and top sellers for World Cup 2026 fans",
        buyOnAmazon: "Buy on Amazon",
        viralTikTok: "Viral on TikTok",
        topSeller: "Top Seller",
      },
      // Travel
      travel: {
        sectionLabel: "Travel Guide",
        title: "Plan Your World Cup Trip",
        subtitle: "Book your match day stays, car rentals, and flights to USA, Canada & Mexico for the World Cup 2026",
        matchDayStays: "Match Day Stays",
        matchDayStaysDesc: "Book premium hotels near World Cup stadiums across USA, Canada & Mexico. Best rates guaranteed.",
        findHotels: "Find Hotels",
        carRentals: "Car Rentals",
        carRentalsDesc: "Explore host cities in style. Rent a car and drive between match venues across North America.",
        rentCar: "Rent a Car",
        flights: "Flights to Host Cities",
        flightsDesc: "Find the best flight deals to New York, Los Angeles, Miami, Toronto, Mexico City & more.",
        searchFlights: "Search Flights",
        hostCities: "Host Cities - USA, Canada & Mexico",
      },
      // Schedule
      schedule: {
        sectionLabel: "Match Schedule",
        title: "World Cup 2026 Schedule",
        subtitle: "Key matches across USA, Canada & Mexico. Full schedule to be announced closer to the tournament.",
        disclaimer: "* Schedule is preliminary. Official FIFA World Cup 2026 schedule to be confirmed.",
        openingMatch: "Opening Match",
        groupStage: "Group Stage",
        quarterFinal: "Quarter Final",
        semiFinal: "Semi Final",
        final: "Final",
      },
      // Countdown
      countdown: {
        limitedOffer: "Limited World Cup Offer",
        endsIn: "Ends in:",
        days: "Days",
        hours: "Hours",
        mins: "Mins",
        secs: "Secs",
      },
      // Common
      common: {
        scrollTop: "Back to top",
        loading: "Loading...",
      },
      // Footer
      footer: {
        tagline: "Your premium destination for World Cup 2026 fan gear, travel deals, and match schedules.",
        quickLinks: "Quick Links",
        support: "Support",
        copyright: "© 2026 FanEdge. All rights reserved.",
        affiliate: "Affiliate Disclosure: We earn from qualifying purchases.",
      },
    },
  },
  ar: {
    translation: {
      // Navigation
      nav: {
        home: "الرئيسية",
        fansGear: "منتجات المشجعين",
        travelGuide: "دليل السفر",
        schedule: "جدول كأس العالم",
        joinNow: "انضم الآن",
      },
      // Hero
      hero: {
        yourPremium: "وجهتك المميزة",
        worldCup2026: "كأس العالم 2026",
        destination: "لكل مشجع",
        subtitle: "تسوق المنتجات الرائجة على تيك توك، والأكثر مبيعاً، واحجز إقامتك في أمريكا وكندا والمكسيك. كل ما يحتاجه المشجع الحقيقي.",
        shopGear: "تسوق المنتجات",
        planTrip: "خطط رحلتك",
        scrollExplore: "مرر للاستكشاف",
      },
      // Products
      products: {
        sectionLabel: "منتجات مميزة",
        title: "مجموعة منتجات المشجعين",
        subtitle: "مجموعة مختارة من المنتجات الرائجة على تيك توك والأكثر مبيعاً لمشجعي كأس العالم 2026",
        buyOnAmazon: "اشتري من أمازون",
        viralTikTok: "رائج على تيك توك",
        topSeller: "الأكثر مبيعاً",
      },
      // Travel
      travel: {
        sectionLabel: "دليل السفر",
        title: "خطط رحلة كأس العالم",
        subtitle: "احجز إقامتك يوم المباراة وتأجير السيارات والرحلات إلى أمريكا وكندا والمكسيك لكأس العالم 2026",
        matchDayStays: "إقامة يوم المباراة",
        matchDayStaysDesc: "احجز فنادق فاخرة بالقرب من ملاعب كأس العالم. أفضل الأسعار مضمونة.",
        findHotels: "ابحث عن فنادق",
        carRentals: "تأجير السيارات",
        carRentalsDesc: "استكشف المدن المستضيفة بأناقة. استأجر سيارة وتنقل بين الملاعب.",
        rentCar: "استأجر سيارة",
        flights: "رحلات للمدن المستضيفة",
        flightsDesc: "اعثر على أفضل عروض الطيران إلى نيويورك ولوس أنجلوس وميامي وتورونتو ومكسيكو سيتي.",
        searchFlights: "ابحث عن رحلات",
        hostCities: "المدن المستضيفة - أمريكا وكندا والمكسيك",
      },
      // Schedule
      schedule: {
        sectionLabel: "جدول المباريات",
        title: "جدول كأس العالم 2026",
        subtitle: "المباريات الرئيسية في أمريكا وكندا والمكسيك. سيتم الإعلان عن الجدول الكامل قريباً.",
        disclaimer: "* الجدول مبدئي. سيتم تأكيد جدول كأس العالم الرسمي من الفيفا.",
        openingMatch: "مباراة الافتتاح",
        groupStage: "دور المجموعات",
        quarterFinal: "ربع النهائي",
        semiFinal: "نصف النهائي",
        final: "النهائي",
      },
      // Countdown
      countdown: {
        limitedOffer: "عرض كأس العالم المحدود",
        endsIn: "ينتهي في:",
        days: "أيام",
        hours: "ساعات",
        mins: "دقائق",
        secs: "ثواني",
      },
      // Common
      common: {
        scrollTop: "العودة للأعلى",
        loading: "جاري التحميل...",
      },
      // Footer
      footer: {
        tagline: "وجهتك المميزة لمنتجات مشجعي كأس العالم 2026 وعروض السفر وجداول المباريات.",
        quickLinks: "روابط سريعة",
        support: "الدعم",
        copyright: "© 2026 فان إيدج. جميع الحقوق محفوظة.",
        affiliate: "إفصاح الشراكة: نحصل على عمولات من المشتريات المؤهلة.",
      },
    },
  },
  es: {
    translation: {
      // Navigation
      nav: {
        home: "Inicio",
        fansGear: "Equipamiento Fan",
        travelGuide: "Guía de Viaje",
        schedule: "Calendario Mundial",
        joinNow: "Únete Ahora",
      },
      // Hero
      hero: {
        yourPremium: "Tu Destino",
        worldCup2026: "Mundial 2026",
        destination: "Premium",
        subtitle: "Compra productos virales de TikTok, los más vendidos, y reserva tu estadía en USA, Canadá y México. Todo lo que un verdadero fan necesita.",
        shopGear: "Comprar Equipamiento",
        planTrip: "Planifica tu Viaje",
        scrollExplore: "Desplázate para explorar",
      },
      // Products
      products: {
        sectionLabel: "Productos Destacados",
        title: "Colección de Equipamiento Fan",
        subtitle: "Selección de productos virales de TikTok y más vendidos para fans del Mundial 2026",
        buyOnAmazon: "Comprar en Amazon",
        viralTikTok: "Viral en TikTok",
        topSeller: "Más Vendido",
      },
      // Travel
      travel: {
        sectionLabel: "Guía de Viaje",
        title: "Planifica tu Viaje al Mundial",
        subtitle: "Reserva estadías, alquiler de autos y vuelos a USA, Canadá y México para el Mundial 2026",
        matchDayStays: "Estadías Día de Partido",
        matchDayStaysDesc: "Reserva hoteles premium cerca de estadios del Mundial. Mejores tarifas garantizadas.",
        findHotels: "Buscar Hoteles",
        carRentals: "Alquiler de Autos",
        carRentalsDesc: "Explora ciudades sede con estilo. Alquila un auto y conduce entre sedes.",
        rentCar: "Alquilar Auto",
        flights: "Vuelos a Ciudades Sede",
        flightsDesc: "Encuentra las mejores ofertas de vuelos a Nueva York, Los Ángeles, Miami, Toronto, Ciudad de México.",
        searchFlights: "Buscar Vuelos",
        hostCities: "Ciudades Sede - USA, Canadá y México",
      },
      // Schedule
      schedule: {
        sectionLabel: "Calendario de Partidos",
        title: "Calendario Mundial 2026",
        subtitle: "Partidos clave en USA, Canadá y México. Calendario completo por anunciar.",
        disclaimer: "* Calendario preliminar. Calendario oficial FIFA por confirmar.",
        openingMatch: "Partido Inaugural",
        groupStage: "Fase de Grupos",
        quarterFinal: "Cuartos de Final",
        semiFinal: "Semifinal",
        final: "Final",
      },
      // Countdown
      countdown: {
        limitedOffer: "Oferta Mundial Limitada",
        endsIn: "Termina en:",
        days: "Días",
        hours: "Horas",
        mins: "Mins",
        secs: "Segs",
      },
      // Common
      common: {
        scrollTop: "Volver arriba",
        loading: "Cargando...",
      },
      // Footer
      footer: {
        tagline: "Tu destino premium para equipamiento fan del Mundial 2026, ofertas de viaje y calendarios.",
        quickLinks: "Enlaces Rápidos",
        support: "Soporte",
        copyright: "© 2026 FanEdge. Todos los derechos reservados.",
        affiliate: "Divulgación: Ganamos comisiones por compras calificadas.",
      },
    },
  },
  fr: {
    translation: {
      // Navigation
      nav: {
        home: "Accueil",
        fansGear: "Équipement Fan",
        travelGuide: "Guide de Voyage",
        schedule: "Calendrier Mondial",
        joinNow: "Rejoindre",
      },
      // Hero
      hero: {
        yourPremium: "Votre Destination",
        worldCup2026: "Coupe du Monde 2026",
        destination: "Premium",
        subtitle: "Achetez des produits viraux TikTok, les meilleures ventes, et réservez votre séjour aux USA, Canada et Mexique. Tout ce dont un vrai fan a besoin.",
        shopGear: "Acheter Équipement",
        planTrip: "Planifier Voyage",
        scrollExplore: "Défiler pour explorer",
      },
      // Products
      products: {
        sectionLabel: "Produits Vedettes",
        title: "Collection Équipement Fan",
        subtitle: "Sélection de produits viraux TikTok et meilleures ventes pour fans de la Coupe du Monde 2026",
        buyOnAmazon: "Acheter sur Amazon",
        viralTikTok: "Viral sur TikTok",
        topSeller: "Meilleure Vente",
      },
      // Travel
      travel: {
        sectionLabel: "Guide de Voyage",
        title: "Planifiez Votre Voyage Mondial",
        subtitle: "Réservez hébergements, locations de voitures et vols vers USA, Canada et Mexique pour la Coupe du Monde 2026",
        matchDayStays: "Séjours Jour de Match",
        matchDayStaysDesc: "Réservez des hôtels premium près des stades. Meilleurs tarifs garantis.",
        findHotels: "Trouver Hôtels",
        carRentals: "Location de Voitures",
        carRentalsDesc: "Explorez les villes hôtes avec style. Louez une voiture entre les sites.",
        rentCar: "Louer Voiture",
        flights: "Vols vers Villes Hôtes",
        flightsDesc: "Trouvez les meilleures offres de vols vers New York, Los Angeles, Miami, Toronto, Mexico.",
        searchFlights: "Rechercher Vols",
        hostCities: "Villes Hôtes - USA, Canada et Mexique",
      },
      // Schedule
      schedule: {
        sectionLabel: "Calendrier des Matchs",
        title: "Calendrier Coupe du Monde 2026",
        subtitle: "Matchs clés aux USA, Canada et Mexique. Calendrier complet à annoncer.",
        disclaimer: "* Calendrier préliminaire. Calendrier officiel FIFA à confirmer.",
        openingMatch: "Match d'Ouverture",
        groupStage: "Phase de Groupes",
        quarterFinal: "Quart de Finale",
        semiFinal: "Demi-Finale",
        final: "Finale",
      },
      // Countdown
      countdown: {
        limitedOffer: "Offre Mondiale Limitée",
        endsIn: "Se termine dans:",
        days: "Jours",
        hours: "Heures",
        mins: "Mins",
        secs: "Secs",
      },
      // Common
      common: {
        scrollTop: "Retour en haut",
        loading: "Chargement...",
      },
      // Footer
      footer: {
        tagline: "Votre destination premium pour l'équipement fan Coupe du Monde 2026, offres de voyage et calendriers.",
        quickLinks: "Liens Rapides",
        support: "Support",
        copyright: "© 2026 FanEdge. Tous droits réservés.",
        affiliate: "Divulgation: Nous gagnons des commissions sur les achats qualifiés.",
      },
    },
  },
};
